...
from src.models.box_office_regression.pipelines.training_pipeline import (
    ...
    BoxOfficeRegressionTrainingPipeline
)
from src.utilities.plot import plot_multi_line_graph


class BoxOfficeRegressionModelHandler(RegressionModelHandler[BoxOfficeRegressionConfigDict]):
    """
    Handles CLI commands related to the Box Office Regression Model.

    This class extends `BoxOfficeRegressionModelHandler` to provide specific implementations
    for training, predicting with, and evaluating the Box Office Regression Model.
    It manages the complete lifecycle for this specific model type.

    :cvar _EVALUATION_CACHE_FILE_NAME: The filename for the Box Office Regression Model's evaluation cache.
    """
    ...
    def __init__(self, parser: ArgumentParser) -> None:
        ...
        super().__init__(
            ...
            model_type=ProjectModelType.BOX_OFFICE_REGRESSION,
            evaluator=BoxOfficeRegressionEvaluator()
        )

    @override
    def train(self, args: Namespace) -> None:
        """
        Orchestrates the Box Office Regression Model training process.
        ...
        """
        effective_config: BoxOfficeRegressionConfigDict = self._prepare_training_config(args=args)

        try:
            artifacts_folder: Path = ProjectPaths.get_model_root_path(
                model_id=args.model_id, model_type=self._model_type
            )
            pipeline_config: BoxOfficeRegressionPipelineConfig = BoxOfficeRegressionPipelineConfig(**effective_config)
            data_processor: BoxOfficeRegressionDataProcessor = BoxOfficeRegressionDataProcessor(
                model_artifacts_path=artifacts_folder)
            model_core: BoxOfficeRegressionModelCore = BoxOfficeRegressionModelCore()
            pipeline: BoxOfficeRegressionTrainingPipeline = BoxOfficeRegressionTrainingPipeline(
    ...
    def _get_default_config_filename(self) -> str:
        """
        Gets the filename for the Box Office Regression Model's default configuration.

        :returns: The name of the default configuration file.
        """
        return "box_office_regression_defaults.yaml"

    @override
    def _get_evaluation_cache_filename(self) -> str:
        """
        Gets the filename for the Box Office Regression Model's evaluation cache.
    ...
    @override
    def _run_evaluation_for_epoch(self,
                                  eval_config: BoxOfficeRegressionEvaluationConfig) -> BoxOfficeRegressionEvaluationResult:
        """
        Runs the evaluation for a single epoch using the evaluator of Box Office Regression Model.
    ...
    def _plot_specific_graphs(
        ...
        """
        Plots graphs specific to the Box Office Regression Model.
        ...
        if args.show_f1_score:
            self._logger.info("Plotting specific metric of Box Office Regression Model: Range F1-Score.")

            prediction_results: list[BoxOfficeRegressionEvaluationResult] = [
                res for res in eval_results if isinstance(res, BoxOfficeRegressionEvaluationResult)
            ]

            if not prediction_results:
                self._logger.warning(
                    "No evaluation results of Box Office Regression Model found. Cannot plot F1-score.")
                return
    ...
    def _display_specific_metrics(self, result: BaseEvaluationResult, args: Namespace, result_logger: Logger) -> None:
        """
        Displays metrics specific to the Box Office Regression Model.
        ...
        if isinstance(result, BoxOfficeRegressionEvaluationResult):
    ...
    def _build_evaluation_config(
        self, args: Namespace, original_config_data: BoxOfficeRegressionConfigDict, epoch_to_evaluate: int
    ) -> BoxOfficeRegressionEvaluationConfig:
        """
        Builds the evaluation configuration object for a single epoch.

        This method constructs a `BoxOfficeRegressionEvaluationConfig` object based on the
        ...
        if args.dataset_name:
            ...
            return BoxOfficeRegressionEvaluationConfig(
